...
class FeatureManager:
    ...
    def fit(self, df_train: gpd.GeoDataFrame) -> Self:
        ...
        daily_t = (
            df_train.assign(date=pd.to_datetime(df_train["start_date"]).dt.normalize())
            .groupby("date")["temperature_2m_mean"]
            .mean()
        )
    ...
    def transform(self, df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        w["precip_sum_roll3"] = (
            w["precipitation_sum"].shift(1).rolling(3, min_periods=1).sum()
        )
    ...
    def _dimension_features(self, df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        df["date"] = pd.to_datetime(
            df["date"] if "date" in df.columns else df["start_date"]
        ).dt.normalize()

        y_cnt_by_day = (
            df.groupby(["lor", "date"]).size().sort_index()
        )  # MultiIndex Series

        #  7-day past mean for each LOR
        y_roll7_by_day = (
            y_cnt_by_day.groupby(level=0)
    ...
    def _history_features(self, df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        mi_df = pd.MultiIndex.from_frame(df[["lor", "date"]])
        acc: Dict[str, List] = {
            k: [] for k in ["y_lag1", "y_lag7", "y_roll7", "y_diff1", "zero_streak"]
        }
        ...
        df["pop_density_km2"] = (df["population_total"] / df["area_km2"]).replace(
    ...
    def _events_content_features(self, df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        for t in top_types:
            ...
            df[col] = (
                (df["bicycle_type"].astype(str).eq(t).astype(int))
                .groupby(gkey)
                .transform("mean")
            )
