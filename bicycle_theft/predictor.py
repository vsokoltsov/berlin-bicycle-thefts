...
class Predictor:
    ...
    def build(self, items: List[PredictItem]) -> pd.DataFrame:
        ...
        df["lor"] = (
            df["lor"].astype(str).str.replace(r"\D", "", regex=True).str.zfill(8)
        )

        df_items = pd.DataFrame(
            [it.model_dump() for it in items], columns=["lor", "date"]
        )
        df_items["lor"] = (
            df_items["lor"].astype(str).str.replace(r"\D", "", regex=True).str.zfill(8)
        )
        df_items["date"] = (
            pd.to_datetime(df_items["date"], errors="coerce")
            .dt.tz_localize(None)
            .dt.normalize()
        )
        ...
        df_full = df_full.sort_values(["lor", "date"]).drop_duplicates(
            ["lor", "date"], keep="last"
        )
        fm.load_state(
            {
                "imp_median": self.artifacts["median_imputer"],
                "price_median_clip": self.artifacts["median_imputer"][
                    "price_median_clip"
                ],
        ...
        X_all["lor"] = (
            X_all["lor"].astype(str).str.replace(r"\D", "", regex=True).str.zfill(8)
        )
        X_all["date"] = (
            pd.to_datetime(X_all["date"], errors="coerce")
            .dt.tz_localize(None)
            .dt.normalize()
        )
        ...
        pred_df = df_items.merge(
            X_all, on=["lor", "date"], how="left", sort=False, validate="one_to_one"
        )
        if zero_like_cols:
    ...
    def _build_context_panel(
        ...
        df_events["date"] = (
            pd.to_datetime(df_events["start_date"]).dt.tz_localize(None).dt.normalize()
        )
        df_events["lor"] = (
            df_events["lor"].astype(str).str.replace(r"\D", "", regex=True).str.zfill(8)
        )

        df_items["date"] = (
            pd.to_datetime(df_items["date"]).dt.tz_localize(None).dt.normalize()
        )
        df_items["lor"] = (
            df_items["lor"].astype(str).str.replace(r"\D", "", regex=True).str.zfill(8)
        )
        ...
        panel = pd.MultiIndex.from_product(
            [lors, dates], names=["lor", "date"]
        ).to_frame(index=False)

        daily_ev = (
            df_events.groupby(["lor", "date"]).size().rename("y_count").reset_index()
        )
        panel = panel.merge(daily_ev, on=["lor", "date"], how="left")
        panel["y_count"] = panel["y_count"].fillna(0.0)

        weather_df["date"] = (
            pd.to_datetime(weather_df["date"]).dt.tz_localize(None).dt.normalize()
        )
